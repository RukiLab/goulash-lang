...
- `imeclause()`（GUI）：変換中の文節を `[文字列, 開始, 終了]` で取得。開始・終了は `imeget()` 文字列へのルーン位置。未変換・範囲なしでは全ゼロ
...
- `objprm(id, "textcolor"|"backcolor", 0xRRGGBB)`（GUI）：全ウィジェットの文字色・背景色を変更（`-1` で既定に戻す。`"color"`・`"bgcolor"` は別名）

### 変更（破壊的）
- `mes()` / `print()` の文字を図形と同じキャンバスレイヤーに描画するように変更（HSP と同じ。`gsel()` で選んだバッファへ `pset()` / `line()` / `boxf()` / `circle()` 等と一緒に焼き込まれ、重なりは呼び出し順で決まる。以降の `boxf()` で文字が隠れ、`boxf()` の後に `mes()` すれば文字が図形の上に乗る。従来は文字が常に図形の上に別レイヤーで描かれていた）。`cls()` は文字と図形をまとめて消し、カーソルが画面下端を越えると文字と図形が一緒に1行分スクロールする
